...
恢复清单记录原表、生成 Base、源文件和初始记录路径。恢复只替换当前匹配 ID 的插件 Base，不删除记录。成员资格不查询路径，因此记录重命名或移动无需事件监听。插件的新建记录命令根据当前宿主笔记目录计算收件箱；宿主移动不触碰已有记录。运行时同时读取 `structural-tables` 和旧版 `structural_table_ids`；双字段等价时接受，无效或冲突时停止处理。显式的全库迁移逐文件预览计划动作，在每次 Base 重写前再次核对完整源码，更新使用 LF、CRLF 或 CR 换行的旧成员字段与 Base 过滤条件，并提供默认关闭且只作用于有效成员笔记的 `structural_record_id` 清理；后续写入失败时恢复已经完成的文件。
