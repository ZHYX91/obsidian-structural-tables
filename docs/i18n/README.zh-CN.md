...
- 用把手选中整行或整列，再从右键菜单执行插入、删除、移动、对齐、合并、拆分或设置表头；桌面端仅显示当前悬停、键盘聚焦或已选中的把手，粗指针设备始终显示触摸尺寸的把手。Tab 只进入一次单元格组或把手组，方向键在组内移动。
...
在实时预览中，普通 Markdown 表格默认沿用 Obsidian 原生编辑器。启用“接管普通 Markdown 表格”后，不改变 GFM 源码也能获得与结构表格一致的渲染控件、行列把手、单元格选区、原位编辑、右键菜单、布局、密度和交替行底色；关闭后立即恢复原生行为。已渲染表格保持合并边框，在细指针设备上按内容采用接近原生的行高，在粗指针设备上保留触摸尺寸，并使用跟随主题的语义背景：列表头和左上角表头比行表头更突出。把手覆盖在表格外侧留白中，不会推移表格内容的对齐位置；它们仅按当前悬停、键盘聚焦或已选中的行列逐个显现，不会铺满整个表格。焦点或编辑器光标移开后，插件的单元格选区会清除。向插件接管的单元格粘贴 `[[目标|别名]]` 或 `![[图片|尺寸]]` 时，会自动保存为表格安全的 `[[目标\|别名]]` 与 `![[图片\|尺寸]]`，且不会重复转义。会丢弃非空内容或破坏合并矩形的操作会被拒绝。
...
现有使用 `structural_table_ids` 的 Base 仍可继续使用。执行“迁移旧版 Structural Tables Base 属性…”可逐文件预览计划变更，再替换旧成员属性和 Base 过滤条件；仅具有有效 Structural Tables 成员属性的笔记可选择移除已停用的 `structural_record_id`，且该清理默认关闭。插件不会在启动时自动迁移；旧、新成员属性无效或互相冲突时，迁移会停止且不会覆盖它们。
