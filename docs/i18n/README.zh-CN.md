...
- 支持使用单一稳定焦点边框原位编辑单元格，不会撑宽列，并自动转义粘贴的 Wiki 链接中的 `|`。
- 用把手选中整行或整列，再从右键菜单执行插入、删除、移动、对齐、合并、拆分或设置表头；桌面端将指针移到单元格上会显示对应行、列的把手，键盘聚焦或已选中的把手保持可见，粗指针设备始终显示触摸尺寸的把手。Tab 只进入一次单元格组或把手组，方向键在组内移动。
- 从浏览器和电子表格粘贴 HTML 表格，并保留跨行、跨列结构及单元格内的视觉换行。
...
4. 在桌面端单击或在触屏上双击已渲染的单元格，或选中后按 Enter/F2，可原位编辑；Shift+Enter 或编辑框右键菜单中的“在单元格中插入换行”会插入 `<br>`，Enter 提交，Escape 取消，Tab/Shift+Tab 提交并按源码顺序前往下一个或上一个可见单元格，跳过合并占位格；
5. 使用行列把手、用鼠标拖过多个单元格，或在 Android 上依次点选矩形的首格和末格，再通过右键或长按执行插入、安全删除、移动、对齐、合并、拆分或设置表头；
6. 从浏览器、Excel 或 Google Sheets 粘贴 HTML 表格，可保留受支持的跨行、跨列结构及单元格换行；
7. 从命令面板预览并确认规范格式化，将当前有效表格复制为 HTML、GFM、TSV 或 CSV，预览展开为 GFM 的结果，或迁移 Sheets Extended 行表头分隔列；显式写入表格时会按显示宽度对齐源码管道符（包括中文内容），纯渲染不会改写笔记；
...
单元格内的 `<br>`、`<br/>` 与 `<br />` 都会在阅读视图和插件接管的实时预览表格中显示为视觉换行。手写标签在格式化时保持原样；Shift+Enter、编辑框菜单、多行纯文本粘贴和 HTML 导入统一写入 `<br>`。语义 HTML 导出会生成真正的换行元素，GFM、TSV 与 CSV 这类文本输出则保留标签文本。这是视觉换行语法，不是真正的多行 Markdown 单元格或块级内容。

一旦出现任一结构特性，所有行都必须与分隔行严格等宽。无效结构保留原 Markdown 并显示诊断。“格式化当前结构表格”会先预览规范形式，只有明确确认后才替换源码：左上角保存内容，合并区域首行其余单元格使用 `<`，下方覆盖单元格使用 `^`，源码管道符按终端显示宽度对齐。行表头 `||`、对齐记号、转义管道、Wiki 链接、代码跨度和既有 LF/CRLF/CR 换行均保持有效。原位编辑、合并、拆分、行列操作和导入表格等显式写入也会执行同样的对齐；纯渲染绝不改写 Markdown。转换为 GFM、TSV 或 CSV 时会重复合并值，并用 ` / ` 连接多行列表头路径，使展开结果保持明确。

在实时预览中，普通 Markdown 表格默认沿用 Obsidian 原生编辑器。启用“接管普通 Markdown 表格”后，不改变 GFM 源码也能获得与结构表格一致的渲染控件、行列把手、单元格选区、原位编辑、右键菜单、布局、密度和交替行底色；关闭后立即恢复原生行为。已渲染表格默认跟随原生主题样式，按合并后的逻辑位置保留正确边框，并在粗指针设备上为可交互行保留触摸尺寸。可选的密度和样式覆盖不会改变源码内容。把手覆盖在表格外侧留白中，不会推移表格内容的对齐位置；指针移到单元格上时只显示对应行、列的把手，键盘聚焦或已选中的把手保持可见。焦点或编辑器光标移开后，插件的单元格选区会清除。向插件接管的单元格粘贴 `[[目标|别名]]` 或 `![[图片|尺寸]]` 时，会自动保存为表格安全的 `[[目标\|别名]]` 与 `![[图片\|尺寸]]`，且不会重复转义。会丢弃非空内容或破坏合并矩形的操作会被拒绝。
...
该目录只是创建收件箱，不是成员边界：移动或重命名记录笔记不会改变成员资格。宿主笔记移动后，已有记录保持用户安排的位置。使用生成 Base 的原生“新建”时，插件无需添加记录 ID，也能把刚创建的笔记移入宿主笔记旁的收件箱；如果用户已经主动移动了该笔记，就保留用户选择的位置。“为当前已提升 Base 新建记录”仍可从命令面板和右键菜单使用。使用“从当前已提升 Base 恢复表格”可从 `_promotion.json` 恢复原表格；生成的笔记会刻意保留。提升时，每个非空表头（包括纯数字和带前导零的表头）都会保留为 Property 名；只有空表头使用 `column_n`，重名或保留名追加数字后缀。导入的单元格值按字符串保存，因此前导零和标识符不会变化。

现有使用 `structural_table_ids` 的 Base 仍可继续使用。执行“迁移旧版 Structural Tables Base 属性…”可逐文件预览计划变更，再替换旧成员属性和 Base 过滤条件；仅具有有效 Structural Tables 成员属性的笔记可选择移除已停用的 `structural_record_id`，且该清理默认关闭。切换选项时，清理数量和逐文件动作会同步更新。插件不会在启动时自动迁移；旧、新成员属性无效或互相冲突时，迁移会停止且不会覆盖它们，失败回滚也会保留同时发生的无关编辑。

<!-- section: settings -->
## 设置

设置页沿用 Obsidian 原生控件，分为“常规”“视图”和“外观”三个页签。“常规”控制 HTML 表格粘贴转换和启动冲突提示；“视图”除阅读视图、实时预览和诊断外，还提供默认关闭的普通表格接管。新安装的表格样式、宽度和密度默认跟随主题。“外观”提供“跟随主题”“网格表”“三线表”样式，以及独立的适应内容靠左/居中/铺满正文布局、舒适/紧凑密度和交替行底色；界面语言可跟随 Obsidian，或使用英文/简体中文。

插件接管的表格限制在正文范围内，即使主题启用了宽表格类也不会越界；过宽内容在表格内部横向滚动。原生普通表格继续遵循 Obsidian 和主题，启用“接管普通 Markdown 表格”可让两类表格使用同一套控制。已有明确的宽度与密度偏好会保留；需要原生间距时请选择“跟随主题”。三线表保留顶线、整个表头组下方的一条分隔线和底线，不显示竖线与数据行横线。多级列表头中，有下级表头的跨列标题下方还会显示两端留白的短分组线，不会穿过跨行表头。外观设置仅作用于渲染视图，不改变 Markdown 或导出的 HTML。

<!-- section: limitations -->
## 局限

Structural Tables 不支持公式、单元格样式、块级或真正的多行 Markdown 单元格内容、标题、编号、重复表头源码属性，也不会自动把导入 HTML 单元格内的富文本转换为 Markdown；导入内容会成为纯文本和规范 `<br>` 视觉换行。升级为 Base 会把布局结构展开为 Property：多行列表头路径用 ` / ` 连接，行表头变为普通 Property，合并行表头值写入覆盖的每条记录。数据区合并单元格会阻断确认并标出位置，必须先拆分。原生“新建”收编只处理刚创建且唯一匹配一个 Structural Tables Base 的笔记；有歧义的笔记会保留在原位置。恢复要求生成的 `_promotion.json` 仍位于 Base 代码块记录的路径。解析器会主动拒绝有歧义或非矩形的合并。
