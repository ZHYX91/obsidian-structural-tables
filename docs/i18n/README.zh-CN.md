...
4. 在桌面端单击或在触屏上双击已渲染的单元格，或选中后按 Enter/F2，可原位编辑；Shift+Enter 或编辑框右键菜单中的“在单元格中插入换行”会插入 `<br>`，Enter 提交，Escape 取消，Tab 提交并前往下一格；
...
在实时预览中，普通 Markdown 表格默认沿用 Obsidian 原生编辑器。启用“接管普通 Markdown 表格”后，不改变 GFM 源码也能获得与结构表格一致的渲染控件、行列把手、单元格选区、原位编辑、右键菜单、布局、密度和交替行底色；关闭后立即恢复原生行为。已渲染表格在合并前后保持稳定的舒适或紧凑单元格尺寸与正确边框，在粗指针设备上保留触摸尺寸，并使用跟随主题的语义背景：列表头和左上角表头比行表头更突出。把手覆盖在表格外侧留白中，不会推移表格内容的对齐位置；指针移到单元格上时只显示对应行、列的把手，键盘聚焦或已选中的把手保持可见。焦点或编辑器光标移开后，插件的单元格选区会清除。向插件接管的单元格粘贴 `[[目标|别名]]` 或 `![[图片|尺寸]]` 时，会自动保存为表格安全的 `[[目标\|别名]]` 与 `![[图片\|尺寸]]`，且不会重复转义。会丢弃非空内容或破坏合并矩形的操作会被拒绝。
