...
插件接管的表格限制在正文范围内，即使主题启用了宽表格类也不会越界；过宽内容在表格内部横向滚动。原生普通表格继续遵循 Obsidian 和主题，启用“接管普通 Markdown 表格”可让两类表格使用同一套控制。已有明确的宽度与密度偏好会保留；需要原生间距时请选择“跟随主题”。三线表仅显示顶线、整个表头组下方的一条分隔线和底线，不显示竖线与数据行横线。外观设置仅作用于渲染视图，不改变 Markdown 或导出的 HTML。
