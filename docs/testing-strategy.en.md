...
Verify theme-following, grid, and three-line styles with merged logical edges and multi-row headers. Use real host geometry to check text-column containment, local scrolling, and theme padding; DOM emulators do not fully model cascade layers or CSS variable shorthands.
...
Pure-core tests cover exact numeric, leading-zero, blank, canonical duplicate, non-Latin, and reserved property keys; YAML-quoted property IDs for emoji, whitespace, dots, quotes and backslashes; legacy expression-order recognition; ownership after native YAML saves and multiple views; unmarked recovery with exact manifest proof and conflict refusal; Windows device-name safety with extensions; current and legacy list membership, equivalent dual values, and invalid/conflicting values; deterministic flattening warnings for multi-row/merged column headers and row headers; merged-data blocker coordinates and spans; plugin Base metadata location under LF, CRLF, and CR endings; tilde fences; and ignoring nested examples inside longer fences. Menu tests distinguish ordinary "Upgrade to Base" from structural "Expand structure and upgrade to Base" actions. In-memory Vault transaction tests verify that a blocked plan writes nothing, and cover records and manifest preceding source replacement, no per-record identity writes, retention after partial creation, manifest or editor failure, concurrent edits and same-count child replacement during failure handling, restoration that keeps records, current-folder creation after a host move, missing or mismatched manifests, and target collisions. Migration tests cover per-file actions and toggle-synchronized counts, off-by-default retired-ID cleanup limited to valid membership notes, frontmatter/source stale and conflict refusal, exact concurrent source changes before Base rewriting, filter replacement across all three line endings, and rollback that preserves unrelated concurrent edits after a later file fails.

Regression tests additionally cover edits before an unchanged table, an open draft surviving source shifts, identical tables with shifted indexes, forward and backward traversal through combined row/column spans, and richly formatted row-header source beside a native table. An unmapped raw row-header block must never consume that later native table.

<!-- section: host -->
## Host acceptance

In an explicitly named disposable Vault, test minimum and current Obsidian, light and dark themes, the default theme-following layout within the text column, native-aligned table origin with overlaid handles, theme-following, grid, and three-line styles, span-aware borders on every outer edge, stable comfortable/compact desktop cell dimensions and coarse-pointer touch minimums before and after a merge, zebra continuity through row spans, cell-proxied handle reveal plus focus/selection persistence without whole-table handle exposure or clipping, one Tab stop per cell/row-handle/column-handle group, LTR/RTL arrow navigation and visible focus, selection clearing on focus loss, editor-cursor transitions, and cross-table pointer transfer, Live Preview note opening and cursor transitions, cell edit, Shift+Enter/menu break insertion, multiline paste, Tab and IME behavior, browser/Excel HTML-table paste including in-cell line breaks and disabled pass-through, HTML/GFM/TSV/CSV clipboard results, GFM preview confirm/cancel/stale refusal, Sheets Extended migration, conflict warning, Bases-disabled refusal, both right-click Base-upgrade labels, numeric/leading-zero/blank-header Base promotion, flattening preview, merged-data disabled confirmation with no writes, upgrade confirmation and retained output after failure, generated records and membership, continued results after record move/rename, post-host-move record location, restoration that keeps records, missing-manifest refusal, row/column handles and every menu operation, ordinary-table native behavior while takeover is disabled, ordinary-table ownership in Live Preview and Reading view while enabled, immediate source-preserving release after disabling, settings persistence, undo/redo, and disable cleanup. Repository documentation screenshots use an English disposable Vault, exclude the mouse pointer, and do not count as production deployment evidence.
